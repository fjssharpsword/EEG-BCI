...
def plot_seizure_lead_MNE():
    ...
    fig, ax = plt.subplots(2,1, constrained_layout=True,figsize=(18,18))
    ...
    raw.plot_psd(average=True, ax=ax[0]) #frequency domain

    pA = Patient(id=3) #excluding 12
    clipsA = pA.get_seizure_clips(ch_com)
    sfA = pA.get_sampling_rate()
    pA.close_files()#release files
    ...
    raw = raw.crop(ids[0]/sfA, ids[1]/sfA)
    #raw.plot(show_scrollbars=False, show_scalebars=False)
    raw.plot_psd(average=True, ax=ax[1]) #frequency domain

    ax[0].set_title('Patient A')
    ax[1].set_title('Patient B')
    fig.savefig('/data/pycode/EEG-BCI/CHB-MIT/imgs/spl_psd_mne.png', dpi=300, bbox_inches='tight')
...
def plot_for_net():
    ...
    pA = Patient(id=3) #initiating case
    clipsA = pA.get_seizure_clips(ch_com)
    ...
    #plotting eeg shape 
    fig, ax = plt.subplots(1, constrained_layout=True,figsize=(18,18))
    ax.set_xticks([0,2,4,6,8,10])
    ax.set_xlabel('Times(s)')
    offset = np.arange(0.5, 18, 1)
    ax.set_yticks(offset.tolist(), ch_com)
    ax.set_ylim((0, 18))
    ...
    fig.savefig('/data/pycode/EEG-BCI/CHB-MIT/imgs/net_mne.png', dpi=300, bbox_inches='tight')

...
class singular_spectrum_operator(nn.Module):
    ...
    def _batch_transformer(self, W):
        B, _, N = W.shape
        device = W.get_device()
        #CPU start
        W_ss = self._batch_Hankel_matrix(W.detach().cpu().numpy()) #B*1*N->B*L*K
        W_ss = torch.Tensor(W_ss)
        W_ei = torch.bmm(W_ss, W_ss.permute(0,2,1)) #B*L*K ->B*L*L
        #u, v = self._batch_power_iteration(x_ei)
        #x_ei = torch.bmm(u, v.permute(0, 2, 1))#B* L* L, approximation on first singular value
        evals, evecs = torch.linalg.eig(W_ei) #eigenvalues and eigenvectors
        max_eval, max_evec = evals[:,0].unsqueeze(1).unsqueeze(1), evecs[:,0,:].unsqueeze(1)
        W_ei = torch.bmm(torch.bmm(max_evec.permute(0, 2, 1), max_eval), max_evec)
        W_ss = torch.bmm(W_ei.type(torch.float32), W_ss) #B*L*K
